...
if filter_choice == 1 
    ...
    for m = 0:end_forloop
        ...
        %first buffer
%         if m > 0
%             memorized_buffer(1,1:(delay_coefficient(1,1)*(FL(1,1)*FM(1,1) - 1))) = input_signal((end - (delay_coefficient(1,1)*(FL(1,1)*FM(1,1) - 1) + 1)):end,1);
%         end
    ...
    %                   fclose(file);
                    ...
                    if i == 1 && m > 0 %Copy all needed output values to memorized_buffer
                        memorized_buffer((1 + compute_offset_for_memorized_buffer(i, bestPerm, delay_coefficient)):(mem_len + compute_offset_for_memorized_buffer(i, bestPerm, delay_coefficient))) = memorized_buffer_output((1 + compute_offset_for_memorized_buffer(i, bestPerm, delay_coefficient)):(mem_len + compute_offset_for_memorized_buffer(i, bestPerm, delay_coefficient)));
                    end
...
%                         signal = input_signal; %true only for the first input
                        ...
                        memorized_buffer_output((1 + compute_offset_for_memorized_buffer(i, bestPerm, delay_coefficient)):(compute_offset_for_memorized_buffer(i, bestPerm, delay_coefficient) + current_mem_len)) = input_signal((size_buffer - current_mem_len + 1):size_buffer); 
                    end
                    ...
                    delayed_buffer = delay_buffer(signal, memorized_buffer, a*(FL(1,i)*FM(1,i) - 1), delay_coefficient, k - 1, i, bestPerm);
